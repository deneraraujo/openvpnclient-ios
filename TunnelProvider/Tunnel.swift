...
open class Tunnel: NSObject {
    ...
    func writeDataToTunnel(_ data: Data, startingAtOffset: Int) -> Int {
        Util.output("[tunnel,info] writeDataToTunnel called on abstract base class")
        return -1
    }
	...
	func serializeMessage(_ messageProperties: [String: AnyObject]) -> Data? {
		...
		do {
            ...
            Util.output("[tunnel,error] Failed to create a data object from a message property list: \(messageProperties)")
		}
	...
	func sendMessage(_ messageProperties: [String: AnyObject]) -> Bool {
        ...
        guard let messageData = serializeMessage(messageProperties) else {
            Util.output("[tunnel,error] Failed to create message data")
            return false
        }
	...
	func sendData(_ data: Data, forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
            Util.output("[tunnel,error] Failed to send a data message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendSuspendForConnection(_ connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			Util.output("[tunnel,error] Failed to send a suspend message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendResumeForConnection(_ connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			Util.output("[tunnel,error] Failed to send a resume message for connection \(connectionIdentifier)")
		}
	}
	...
	open func sendCloseType(_ type: TunnelConnectionCloseDirection, forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			Util.output("[tunnel,error] Failed to send a close message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendPackets(_ packets: [Data], protocols: [NSNumber], forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			Util.output("[tunnel,error] Failed to send a packet message")
		}
	}
	...
	func handlePacket(_ packetData: Data) -> Bool {
		...
		do {
			...
			Util.output("[tunnel,error] Failed to create the message properties from the packet")
			return false
		}

		guard let command = properties[TunnelMessageKey.Command.rawValue] as? Int else {
			Util.output("[tunnel,error] Message command type is missing")
			return false
		}
		guard let commandType = TunnelCommand(rawValue: command) else {
			Util.output("[tunnel,error] Message command type \(command) is invalid")
			return false
		}
		...
		switch commandType {
			case .data:
				...
				{
                    Util.output("[tunnel,info] Received data for connection \(String(describing: connection?.identifier)) from \(host):\(port)")
			...
			case .close:
				...
				{
                    Util.output("[tunnel,info] \(String(describing: connection?.identifier)): closing \(closeDirection)")
					targetConnection.closeConnection(closeDirection)
				} else {
                    Util.output("[tunnel,info] \(String(describing: connection?.identifier)): closing reads and writes")
	...
	func handleMessage(_ command: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		Util.output("[tunnel,info] handleMessage called on abstract base class")
		return false
	}
}
