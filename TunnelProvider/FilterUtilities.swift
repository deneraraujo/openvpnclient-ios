...
open class FilterUtilities {
	...
	open class func getRule(_ flow: NEFilterFlow) -> (FilterRuleAction, String, [String: AnyObject]) {
        ...
        guard let hostNameRule = (Util.appGroupDefaults.object(forKey: "rules") as AnyObject).object(forKey: hostname) as? [String: AnyObject] else {
            Util.output("[tunnel,info]\(hostname) is set for NO RULES")
	...
	open class func fetchRulesFromServer(_ serverAddress: String?) {
        Util.output("[tunnel,info] fetch rules called")

		guard serverAddress != nil else { return }
        Util.output("[tunnel,info] Fetching rules from \(String(describing: serverAddress))")

		guard let infoURL = URL(string: "http://\(serverAddress!)/rules/") else { return }
        Util.output("[tunnel,info] Rules url is \(infoURL)")
		...
		do {
            ...
            Util.output("[tunnel,error] Failed to fetch the rules from \(infoURL)")
			return
		}

		let contentArray = content.components(separatedBy: "<br/>")
        Util.output("[tunnel,debug] Content array is \(contentArray)")
        ...
        Util.appGroupDefaults.setValue(urlRules, forKey:"rules")
	}
}
